...
var chargeProvider = {
  get: async (runtime, _message) => {
    ...
    elizaLogger3.log("Charges:", formattedCharges);
...
var invokeContractAction = {
  ...
  handler: async (runtime, _message, state, _options, callback) => {
    ...
    try {
      ...
      const { contractAddress, method: method2, args, amount, assetId, networkId } = invocationDetails.object;
      const wallet = await initializeWallet(runtime, networkId);
...
var readContractAction = {
  ...
  validate: async (runtime, _message) => {
    elizaLogger5.log("Validating runtime for READ_CONTRACT...");
    return !!(runtime.character.settings.secrets?.COINBASE_API_KEY || process.env.COINBASE_API_KEY) && !!(runtime.character.settings.secrets?.COINBASE_PRIVATE_KEY || process.env.COINBASE_PRIVATE_KEY);
  },
  handler: async (runtime, _message, state, _options, callback) => {
    elizaLogger5.log("Starting READ_CONTRACT handler...");
    try {
      Coinbase6.configure({
        apiKeyName: runtime.getSetting("COINBASE_API_KEY") ?? process.env.COINBASE_API_KEY,
        privateKey: runtime.getSetting("COINBASE_PRIVATE_KEY") ?? process.env.COINBASE_PRIVATE_KEY
      ...
      const readDetails = await generateObjectV24({
        runtime,
        context,
        modelClass: ModelClass4.SMALL,
      ...
      elizaLogger5.log("Reading contract:", { contractAddress, method: method2, args, networkId, abi });
...
import {
  ...
  generateObjectV2 as generateObjectV26,
...
async function hasEnoughBalance(client, currency, amount, side) {
  try {
    ...
    elizaLogger7.info(`Checking balance for ${side} order of ${amount} ${checkCurrency}`);
    ...
    elizaLogger7.info(`Required amount (including buffer): ${requiredAmount} ${checkCurrency}`);
...
var executeAdvancedTradeAction = {
  ...
  handler: async (runtime, _message, state, _options, callback) => {
    ...
    let tradeDetails;
    try {
      tradeDetails = await generateObjectV26({
        runtime,
        context: composeContext6({ state, template: advancedTradeTemplate }),
    ...
    try {
      ...
      elizaLogger7.info("Order configuration created:", orderConfiguration);
    } catch (error) {
