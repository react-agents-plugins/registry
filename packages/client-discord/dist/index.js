...
var MessageManager = class {
  ...
  async handleMessage(message) {
    ...
    if (this.runtime.character.clientConfig?.discord?.isPartOfTeam && !this.runtime.character.clientConfig?.discord?.shouldRespondOnlyToMentions) {
      ...
      if (hasInterest && !isDirectlyMentioned) {
        ...
        const lastSelfSortedMemories = lastSelfMemories?.filter((m) => m.userId === this.runtime.agentId).sort(
  ...
  _isMessageForMe(message) {
    ...
    return isMentioned || !this.runtime.character.clientConfig?.discord?.shouldRespondOnlyToMentions && (message.content.toLowerCase().includes(this.client.user?.username.toLowerCase()) || message.content.toLowerCase().includes(this.client.user?.tag.toLowerCase()) || nickname && message.content.toLowerCase().includes(nickname.toLowerCase()));
  }
  ...
  async _shouldIgnore(message) {
    ...
    if (this.runtime.character.clientConfig?.discord?.isPartOfTeam) {
      ...
      if (this._isTeamLeader()) {
        ...
        const randomDelay = Math.floor(Math.random() * (TIMING_CONSTANTS.TEAM_MEMBER_DELAY_MAX - TIMING_CONSTANTS.TEAM_MEMBER_DELAY_MIN)) + TIMING_CONSTANTS.TEAM_MEMBER_DELAY_MIN;
  ...
  async _shouldRespond(message, state) {
    ...
    try {
      if (this.runtime.character.clientConfig?.discord?.isPartOfTeam) {
        ...
        if (this._isTeamLeader() && !this._isRelevantToTeamMember(message.content, message.channelId)) {
          const randomDelay = Math.floor(Math.random() * (TIMING_CONSTANTS.LEADER_DELAY_MAX - TIMING_CONSTANTS.LEADER_DELAY_MIN)) + TIMING_CONSTANTS.LEADER_DELAY_MIN;
          await new Promise((resolve) => setTimeout(resolve, randomDelay));
          if (channelState?.messages?.length) {
    ...
    if (channelState?.previousContext) {
      const shouldRespondContext2 = await this._shouldRespondBasedOnContext(
        message,
        channelState
      );
