...
import { Connection as Connection9, PublicKey as PublicKey8, Transaction } from "@solana/web3.js";
async function invokeSwapDao(connection3, authority, statePDA, walletPDA, instructionData) {
  ...
  const transaction = new Transaction().add({
    programId: new PublicKey8("PROGRAM_ID"),
    keys: [
...
var executeSwapForDAO = {
  ...
  handler: async (runtime, message) => {
    ...
    try {
      ...
      const daoMint = new PublicKey8(runtime.getSetting("DAO_MINT"));
      const [statePDA] = await PublicKey8.findProgramAddress(
        [Buffer.from("state"), daoMint.toBuffer()],
        authority.publicKey
      );
      const [walletPDA] = await PublicKey8.findProgramAddress(
...
import { generateObjectDEPRECATED as generateObjectDEPRECATED3 } from "@ai16z/eliza";
...
var transfer_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      const mintPubkey = new PublicKey9(content.tokenAddress);
      const recipientPubkey = new PublicKey9(content.recipient);
