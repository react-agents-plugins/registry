...
var continueAction = {
  ...
  handler: async (runtime, message, state, options, callback) => {
    ...
    const messageExists = state.recentMessagesData.filter((m) => m.userId === runtime.agentId).slice(0, maxContinuesInARow + 1).some((m) => m.content === message.content);
    if (messageExists) {
      return;
    }
    await callback(response);
    if (response.action === "CONTINUE") {
      const agentMessages = state.recentMessagesData.filter((m) => m.userId === runtime.agentId).map((m) => m.content.action);
      const lastMessages = agentMessages.slice(0, maxContinuesInARow);
      if (lastMessages.length >= maxContinuesInARow) {
        const allContinues = lastMessages.every(
          (m) => m === "CONTINUE"
...
import { generateTrueOrFalse as generateTrueOrFalse2 } from "@ai16z/eliza";
import { booleanFooter as booleanFooter2 } from "@ai16z/eliza";
import {
...
import { generateTrueOrFalse as generateTrueOrFalse3 } from "@ai16z/eliza";
import { booleanFooter as booleanFooter3 } from "@ai16z/eliza";
import {
...
import { generateTrueOrFalse as generateTrueOrFalse4 } from "@ai16z/eliza";
import { booleanFooter as booleanFooter4 } from "@ai16z/eliza";
import {
...
import { generateTrueOrFalse as generateTrueOrFalse5 } from "@ai16z/eliza";
import { booleanFooter as booleanFooter5 } from "@ai16z/eliza";
import {
...
import { composeContext as composeContext7 } from "@ai16z/eliza";
